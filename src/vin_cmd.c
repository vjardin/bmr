...
int
cmd_vin(int fd, int argc, char *const *argv, int pretty) {
  if (argc == 0) {
    usage_vin();
    return 2;
  }
  ...
  if (!strcmp(argv[0], "get")) {
    ...
    json_object_set_new(o, "VIN_ON_raw", json_integer((uint16_t) won));
    json_object_set_new(o, "VIN_OFF_raw", json_integer((uint16_t) wof));
    if (!raw && have_exp) {
  ...
  if (!strcmp(argv[0], "set")) {
    ...
    if (set_on) {
      if (pmbus_wr_word(fd, PMBUS_VIN_ON, won) < 0) {
        perror("VIN_ON write");
        return 1;
      }
      json_object_set_new(delta, "VIN_ON_raw", json_integer(won));
    }
    if (set_off) {
      if (pmbus_wr_word(fd, PMBUS_VIN_OFF, wof) < 0) {
        perror("VIN_OFF write");
        return 1;
      }
      json_object_set_new(delta, "VIN_OFF_raw", json_integer(wof));
    }

    json_t *after = json_object();
    int rb_on = pmbus_rd_word(fd, PMBUS_VIN_ON);
    int rb_of = pmbus_rd_word(fd, PMBUS_VIN_OFF);
    if (rb_on >= 0)
      json_object_set_new(after, "VIN_ON_raw", json_integer((uint16_t) rb_on));
    if (rb_of >= 0)
      json_object_set_new(after, "VIN_OFF_raw", json_integer((uint16_t) rb_of));
